...
TEST(SingleTersts, CompareToTest) {
...
TEST(SingleTersts, CompareTo2Test) {
	SingleValue	value1(1);
...
TEST(SingleTersts, IsNormalTest) {
...
TEST(SingleTersts, IsPositiveInfinityTest) {
...
TEST(SingleTersts, IsSubnormalTest) {
...
TEST(SingleTersts, TryParseTest) {
	Single	value = 0;
	String	str(L"11");
...
TEST(SingleTersts, TryParse3Test) {
	Single	value = 0;
	String	str(L" 11 ");
	Boolean goodParse = SingleValue::TryParse(&str, &value);
	ASSERT_TRUE(goodParse);
...
TEST(SingleTersts, TryParse4Test) {
	Single	value = 0;
	String	str(L"AAA");
	Boolean goodParse = SingleValue::TryParse(&str, &value);
	ASSERT_TRUE(!goodParse);
}
TEST(SingleTersts, TryParse5Test) {
	Single	value = 0;
	String	str(L"-11");
	Boolean goodParse = SingleValue::TryParse(&str, &value);
	ASSERT_TRUE(goodParse);
